...
from hey_robo.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from hey_robo.config import config  # noqa: F401
from hey_robo.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from hey_robo.tools.background_tool_manager import BackgroundToolManager


logger = logging.getLogger(__name__)


DEFAULT_PROFILES_MODULE = "hey_robo.profiles"
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"hey_robo.tools.{tool_name}"
            try:
                source = _try_load_tool(
                    tool_name,
